...
| 级别 | 话题类型 | 出卡规则 |
|------|---------|---------|
| ⭐ **必出** | **AI 模型更新**（GPT/Claude/Llama/Gemini/DeepSeek 等新版本） | 单卡深度分析 |
| ⭐ **必出** | **AI 开源项目发布**（新框架/工具/库的首次发布或重大版本） | 单卡，含技术亮点 |
| ⭐ **必出** | **AI 相关论文突破**（顶级会议/期刊的重要成果） | 单卡，含可用性判断 |
| 🔵 推荐 | Agent 安全事件（agent 失控/越狱/数据泄露） | 单卡 |
| 🔵 推荐 | 版权/合规判例（训练数据/Agent 责任） | 单卡 |
| ⚪ 可选 | 融资、宏观政策 | 仅当影响上述三个方向时融合 |
| ⚪ 可选 | 公司日常动态 | 不单独出卡，可融合
...
每次生成简报前，**必须先获取当前日期**，用以下方式之一：

- 执行 `date "+%Y-%m-%d"` 获取今天是哪一天
- 或用 `date "+%A"` 判断星期几（决定是否出周末版）
...
**重要：** 检索词中必须包含当前日期或"today"关键词，确保 AnySearch 返回的是今天的信息而非过时内容。
...
**检索策略：**
1. **先用当前日期拼接检索词** —— 例如今天是 2026-08-08，检索词必须包含 `August 8 2026` 或 `2026年8月8日`，避免搜到历史信息
2. 先搜中文 + 英文各一轮，覆盖信息面
3. 每条结果至少看 content 全文（不仅有 snippet），提取关键数字、信源、直接引用
4. 如有偏 finance 的选题（融资/财报/并购），额外用 `finance.news` tag + 设 `params.type=market` 搜索
5. 同主题多条结果时，交叉验证事实一致性，标注冲突点
...
| 标签 | 色值 | 适用话题 |
|------|------|---------|
| ⭐ 模型发布 | `#58a6ff` 蓝 | GPT/Claude/Llama 新版本 |
| ⭐ 开源项目 | `#3fb950` 绿 | 新框架/工具/库发布 |
| ⭐ 论文突破 | `#bc8cff` 紫 | 顶会/顶刊重要成果 |
